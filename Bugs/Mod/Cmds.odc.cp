...
MODULE BugsCmds;
	...
	BEGIN
		...
		WHILE fileList # NIL DO
			IF fileList.type = "bug" THEN
				Strings.Find(fileList.name, "bug$", 0, pos);
	...
	BEGIN
		res := 0;
		IF action.updating OR a.s.eot THEN
	...
	BEGIN
		...
		ELSE
			workersPerChain := 0
		END;
		fileName := outBugDialog.name.item$;
		loc := BugsSerialize.restartLoc;
		f := Files64.dir.New(loc, Files.dontAsk);
		wr.ConnectTo(f);
		wr.SetPos(0);
		BugsSerialize.Externalize(workersPerChain, wr);
		BugsMsg.Lookup("BugsCmds:ModelExternalized", msg);
		BugsFiles.ShowStatus(msg);
		BugsInterface.CheckPoint(wr);
		wr.ConnectTo(NIL);
	...
	BEGIN
		...
		loc := BugsSerialize.restartLoc;
		f := Files64.dir.Old(loc, name + ".bug", Files.shared);
		rd.ConnectTo(f);
		rd.SetPos(0);
		BugsSerialize.Internalize(workersPerChain, rd);
		specificationDialog.numChains := BugsRandnum.numberChains;
		updateDialog.iteration := UpdaterActions.iteration;
		updateDialog.isAdapting := UpdaterActions.endOfAdapting = MAX(INTEGER);
		UpdaterSettings.MarkCompiled;
		UpdaterSettings.FillDialog;
		BugsDialog.UpdateDialogs;
		IF workersPerChain # 0 THEN
			specificationDialog.workersPerChain := workersPerChain;
			Distribute
		END;
		len1 := StdLog.text.Length();
		IF ~BugsCPCompiler.debug THEN StdLog.text.Delete(len0, len1) END;
	...
	PROCEDURE InternalizeModel* (fileName: ARRAY OF CHAR);
		VAR
			len0, len1, workersPerChain: INTEGER;
			loc: Files.Locator;
			f: Files64.File;
			p: ARRAY 1 OF ARRAY 1024 OF CHAR;
			msg: ARRAY 1024 OF CHAR;
	...
	BEGIN
		loc := BugsSerialize.restartLoc;
		f := Files64.dir.Old(loc, fileName + ".bug", Files.shared);
		IF f = NIL THEN
			p[0] := fileName$;
