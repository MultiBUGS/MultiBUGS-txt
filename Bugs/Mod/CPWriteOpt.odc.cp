...
MODULE BugsCPWriteOpt;
	...
	BEGIN
		...
		f.WriteString("SYSTEM, GraphLogical, GraphNodes, GraphRules, GraphScalar, GraphStochastic,  ");
		f.WriteLn;
	...
	BEGIN
		...
		WHILE i < args.numStochs DO
			f.WriteString("p := GraphNodes.Internalize(rd);"); f.WriteLn;
			f.WriteString("node.s"); f.WriteInt(i); f
			.WriteString(" := SYSTEM.ADR(p(GraphStochastic.Node).value);"); f.WriteLn;
			INC(i)
		END;
	...
	BEGIN
		...
		f.WriteString("BEGIN"); f.WriteLn;
		f.WriteString("   RETURN NIL"); f.WriteLn;
	...
	BEGIN
		...
		i := 0;
		WHILE i < args.numStochs DO
			f.WriteString("node.s"); f.WriteInt(i); f.WriteString(" := SYSTEM.ADR(logicalArgs.stochastics[");
			f.WriteInt(i); f.WriteString("].value);"); f.WriteLn;
			INC(i)
		END;
	...
	BEGIN
		...
		END;
		IF args.numStochs > 0 THEN
			f.WriteString("SYSTEM.GET(node.s0, s0);"); f.WriteLn;
			i := 1;
			WHILE i < args.numStochs DO
				f.WriteString("SYSTEM.GET(node.s"); f.WriteInt(i); f.WriteString(" , s"); f.WriteInt(i);
	...
	VAR f: TextMappers.Formatter);
		...
		BEGIN
			INC(top);
			String("SYSTEM.GET(");
			String("node.s"); f.WriteInt(j); String(", "); Val; String(");"); Comment("stochastic"); Ln;
			Active; String(" := node.s"); f.WriteInt(j); String(" = xAdr;"); Ln;
			If; Active; Then;
			Diff; String(" := 1.0 ");
			Else;
			Diff; String(" := 0.0 ");
	...
	BEGIN
		...
		END;
		String("BEGIN"); Ln;
		String("xAdr := SYSTEM.ADR(x(GraphStochastic.Node).value);"); Ln;
		IF args.numLogicals > 0 THEN
			WriteRefLogical(0);
			i := 1;
			WHILE i < args.numLogicals DO
