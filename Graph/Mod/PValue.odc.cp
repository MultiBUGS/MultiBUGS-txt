...
MODULE GraphPValue;
	...
	IMPORT
		Stores,
		GraphFlat, GraphLogical, GraphMultivariate, GraphNodes, GraphRules,
		GraphScalar, GraphStochastic, GraphVector;

	TYPE

		Scalar = POINTER TO ABSTRACT RECORD(GraphScalar.MemNode)
	...
	BEGIN
		...
		WITH prior: GraphMultivariate.Node DO
			...
			i := 0;
			WHILE i < nElem DO
				IF oldValues[i] > prior.components[i].value THEN values[i] := 1 ELSE values[i] := 0 END;
				INC(i);
			END;
			i := 0;
			WHILE i < nElem DO prior.components[i].SetValue(oldValues[i]); INC(i) END
