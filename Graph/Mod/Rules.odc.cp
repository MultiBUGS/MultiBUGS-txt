...
MODULE GraphRules;
	...
	BEGIN
		...
		orF[const, ident] := ident; orF[ident, const] := ident;
		orF[const, prod] := prod; orF[prod, const] := prod;
		orF[const, linear] := linear; orF[linear, const] := linear;
		orF[const, logitLink] := linkFun; orF[logitLink, const] := linkFun;
		orF[const, linkFun] := linkFun; orF[linkFun, const] := linkFun;
		orF[const, expon] := expon; orF[expon, const] := expon;
		orF[const, differ] := differ; orF[differ, const] := differ; 
		orF[ident, ident] := ident;
		orF[ident, prod] := prod; orF[prod, ident] := prod;
		orF[ident, linear] := linear; orF[linear, ident] := linear;
		orF[ident, differ] := differ; orF[differ, ident] := differ; 
		orF[prod, prod] := prod;
		orF[prod, linear] := linear; orF[linear, prod] := linear;
		orF[prod, differ] := differ; orF[differ, prod] := differ; 
		orF[linear, linear] := linear;
		orF[linear, differ] := differ; orF[differ, linear] := differ; 
		orF[logitLink, logitLink] := linkFun;
		orF[logitLink, linkFun] := linkFun; orF[linkFun, logitLink] := linkFun;
		orF[logitLink, differ] := differ; orF[differ, logitLink] := differ; 
		orF[linkFun, linkFun] := linkFun;
		orF[linkFun, differ] := differ; orF[differ, linkFun] := differ;
		orF[expon, differ] := differ; orF[differ, expon] := differ;
		orF[differ, differ] := differ;
	END OrRule;
