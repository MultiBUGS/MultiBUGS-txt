...
To create a dos version of OpenBUGS click into the round blob containing a ! with the mouse

DevLinker.Link
dos ClassicBUGS.exe := BugsClassic$ 1 Bugslogo.ico 


To make a fully linked version of the dynamic link library for Windows click in the round blob below. Modules in red are BlackBox modules. The other modules, the OpenBUGS modules, should occur in the same order as in the make file and not include any of the Windows only modules. Module WinConsole contains Windows IO code. The file /Bugs/Mod/Startup.odc contains the module Startup, hence just Startup in the list of modules to be linked. To make a library that does not use the taucs sparse matrix code remove the modules MathTaucsLib MathTaucsImp from the list of linked modules. 

DevLinker.LinkDynDll
libOpenBUGS.dll := Kernel$+ Files HostFiles StdLoader Meta Math Strings Dialog Stores Services 
...
To create  a fully linked ELF shared library version of OpenBUGSfor Linux click in the round blob.
Modules in red are BlackBox modules. The other modules, the OpenBUGS modules, should occur in the same order as in the make file and not include any of the Windows only modules. Module LinConsole contains Linux IO code. The file /Bugs/Mod/Startup.odc contains the module Startup, hence just Startup in the list of modules to be linked. To make a library that does not use the taucs sparse matrix code remove the modules MathTaucsLib MathTaucsImp from the list of linked modules. 

DevElfLinker.LinkDynDll
libOpenBUGS.so := LinKernel$+ Files LinHostFiles StdLoader Meta Math Strings Dialog Stores Services
