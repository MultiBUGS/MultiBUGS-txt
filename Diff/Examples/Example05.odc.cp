...
#
#
# Example05 from the OpenBUGS differential equation solver document. 
#
# model for the Lotka-Volterra system (compiled component version) 
# with estimation of the initial values 
#
#
...
model
{
  solution[1:ngrid, 1:ndim] <- diff.lotvol(init[1:ndim], 
                                           tgrid[1:ngrid],
                                           parameters[1:nparam], 
                                           origin, 
                                           tol)
  parameters[1] <- exp(log.param[1]) 
  parameters[2] <- exp(log.param[2]) 
  parameters[3] <- exp(log.param[3])
  parameters[4] <- exp(log.param[4])
  log.param[1] ~ dnorm(0.0, 0.0001) 
  log.param[2] ~ dnorm(0.0, 0.0001) 
  log.param[3] ~ dnorm(0.0, 0.0001) 
  log.param[4] ~ dnorm(0.0, 0.0001)
