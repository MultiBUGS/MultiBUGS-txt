...
MODULE PharmacoPKIVbol3ss;
	...
	PROCEDURE Value (params, scalars: GraphNodes.Vector): REAL;
		CONST
			eps = 1.0E-10;
		VAR
			logCL, logQ12, logQ13, logV1, logV2, V3, time, dose, omega, phi,
		...
		BEGIN
			IF t < 0 THEN
				C1 := A / (1 - Math.Exp( - lambda[0] * omega));
				C2 := B / (1 - Math.Exp( - lambda[1] * omega));
				C3 := C / (1 - Math.Exp( - lambda[2] * omega))
			ELSE
				C1 := A * Math.Exp( - lambda[0] * t) / (1 - Math.Exp( - lambda[0] * omega));
				C2 := B * Math.Exp( - lambda[1] * t) / (1 - Math.Exp( - lambda[1] * omega));
				C3 := C * Math.Exp( - lambda[2] * t) / (1 - Math.Exp( - lambda[2] * omega))
			END;
			RETURN dose * (C1 + C2 + C3) / Math.Exp(logV1)
		END ProfileSS;
	...
	BEGIN
		install := "PharmacoPKIVbol3ss.Install"
	END Install;
	...
	BEGIN
		install := "PharmacoPKIVbol3ss.MemInstall"
	END Install;
