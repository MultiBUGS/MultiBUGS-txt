...
MODULE UpdaterUpdaters;
	...
	PROCEDURE (updater: Updater) LogLikelihood* (): REAL, NEW, ABSTRACT;
		...
		(*	node(s) in graphical model that updater updates	*)
	PROCEDURE (updater: Updater) Prior* (index: INTEGER): GraphStochastic.Node, NEW, ABSTRACT;

		(*	sample values for node(s) that updater is associated with	*)
	...
	(*	Sets the global factory object fact for creating updaters	*)
